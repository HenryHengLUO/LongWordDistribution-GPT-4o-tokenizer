...
df_chinese = pd.DataFrame(columns=['Rank', 'Encoding', 'Word Length', 'Word', 'Language Score'])
...
for item in length_dict:
    try:
        ...
        current_lang_class = identifier.classify(current_word)
        current_lang = current_lang_class[0]
        print(current_lang)
        if current_lang_class[1] < 0.5:
            continue
        if current_lang == "en" and Num_engligh <= 100:
            df_english = df_english._append({'Rank': Num_engligh, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_engligh += 1
        elif current_lang == "zh" and Num_chinese <= 100:
            df_chinese = df_chinese._append({'Rank': Num_chinese, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_chinese += 1
        elif current_lang == "ja" and Num_japanese <= 100:
            df_japanese = df_japanese._append({'Rank': Num_japanese, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_japanese += 1
        elif current_lang == "fr" and Num_french <= 100:
            df_french = df_french._append({'Rank': Num_french, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_french += 1
        elif current_lang == "de" and Num_german <= 100:
            df_german = df_german._append({'Rank': Num_german, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_german += 1
        elif current_lang == "es" and Num_spanish <= 100:
            df_spanish = df_spanish._append({'Rank': Num_spanish, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_spanish += 1
        elif current_lang == "it" and Num_italian <= 100:
            df_italian = df_italian._append({'Rank': Num_italian, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_italian += 1
        elif current_lang == "pt" and Num_portuguese <= 100:
            df_portuguese = df_portuguese._append({'Rank': Num_portuguese, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_portuguese += 1
        elif current_lang == "ko" and Num_korean <= 100:
            df_korean = df_korean._append({'Rank': Num_korean, 'Encoding': item, 'Word Length': length_dict[item], 'Word': current_word, 'Language Score':current_lang_class[1]}, ignore_index=True);
            Num_korean += 1
